...
class Ui_Main_KO(object):

        def play_game(self):
                from play_ko import Ui_Play_KO
        ...
        def setupUi(self, Main_KO):
                Main_KO.setObjectName("Main_KO")
                Main_KO.resize(1920, 1080)
                icon = QtGui.QIcon()
                icon.addPixmap(QtGui.QPixmap(":/image/Picture/cards.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                Main_KO.setWindowIcon(icon)
                Main_KO.setStyleSheet("background-image: url(:/image/Picture/k_o.jpg);")
                self.centralwidget = QtWidgets.QWidget(Main_KO)
        ...
        "background: transparent")
                ...
                Main_KO.setCentralWidget(self.centralwidget)
                self.menubar = QtWidgets.QMenuBar(Main_KO)
                self.menubar.setGeometry(QtCore.QRect(0, 0, 1920, 26))
                self.menubar.setObjectName("menubar")
                Main_KO.setMenuBar(self.menubar)
                self.statusbar = QtWidgets.QStatusBar(Main_KO)
                self.statusbar.setObjectName("statusbar")
                Main_KO.setStatusBar(self.statusbar)
                ...
                self.btn_back.clicked.connect(Main_KO.hide)
                self.btn_play.clicked.connect(self.play_game)
                self.btn_play.clicked.connect(Main_KO.hide)

                self.retranslateUi(Main_KO)
                QtCore.QMetaObject.connectSlotsByName(Main_KO)

        def retranslateUi(self, Main_KO):
                _translate = QtCore.QCoreApplication.translate
                Main_KO.setWindowTitle(_translate("Main_KO", "หมวด K-O"))
                self.btn_play.setToolTip(_translate("Main_KO", "<html><head/><body><p>กดปุ่มเพื่อเล่นสุ่มคำศัพท์</p></body></html>"))
                self.btn_save.setToolTip(_translate("Main_KO", "<html><head/><body><p>กดปุ่มเพื่อบันทึกคำศัพท์</p></body></html>"))
                self.btn_delete.setToolTip(_translate("Main_KO", "<html><head/><body><p>กดปุ่มเพื่อลบคำศัพท์</p></body></html>"))
                self.btn_play2.setText(_translate("Main_KO", "PLAY"))
                self.btn_save2.setText(_translate("Main_KO", "ADD&&SAVE"))
                self.btn_delete2.setText(_translate("Main_KO", "DELETE"))
                self.label.setText(_translate("Main_KO", "หมวด K-O"))
                self.btn_show.setText(_translate("Main_KO", "SHOW"))
                self.textbox_delete.setPlaceholderText(_translate("Main_KO", "ตัวอย่าง ant=มด **ใส่=ด้วย"))
                self.btn_search.setToolTip(_translate("Main_KO", "<html><head/><body><p>กดปุ่มเพื่อค้นหา</p></body></html>"))
                self.textbox_save.setPlaceholderText(_translate("Main_KO", "ตัวอย่าง ant=มด **ใส่=ด้วย"))
                self.textbox_search.setPlaceholderText(_translate("Main_KO", "SEARCH"))
                self.btn_back.setToolTip(_translate("Main_KO", "<html><head/><body><p>กดปุ่มเพื่อกลับหน้าแรก</p></body></html>"))
        ...
        def show(self):
                ...
                with open(file,encoding="UTF-8") as f:
                        filesize = os.path.getsize("k_o.txt")
                        if filesize == 0:
                                ...
                                if filesize == 0:
                                        ...
                                        with open(file,encoding="UTF-8") as f:
                                                for line in f:
                                                        line = line.replace('\n','')
                                                        (Vocab1,Vocab2) = line.split('=')
                                                        k_o.setdefault(Vocab1,[])
                                                        k_o[Vocab1].append(Vocab2)
