...
class Ui_Main_UZ(object):

        def play_game(self):
                from play_uz import Ui_Play_UZ
        ...
        def setupUi(self, Main_UZ):
                Main_UZ.setObjectName("Main_UZ")
                Main_UZ.resize(1920, 1080)
                icon = QtGui.QIcon()
                icon.addPixmap(QtGui.QPixmap(":/image/Picture/cards.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                Main_UZ.setWindowIcon(icon)
                Main_UZ.setStyleSheet("background-image: url(:/image/Picture/u_z.jpg);")
                self.centralwidget = QtWidgets.QWidget(Main_UZ)
        ...
        "background: transparent")
                ...
                Main_UZ.setCentralWidget(self.centralwidget)
                self.menubar = QtWidgets.QMenuBar(Main_UZ)
                self.menubar.setGeometry(QtCore.QRect(0, 0, 1920, 26))
                self.menubar.setObjectName("menubar")
                Main_UZ.setMenuBar(self.menubar)
                self.statusbar = QtWidgets.QStatusBar(Main_UZ)
                self.statusbar.setObjectName("statusbar")
                Main_UZ.setStatusBar(self.statusbar)
                ...
                self.btn_back.clicked.connect(Main_UZ.hide)
                self.btn_play.clicked.connect(self.play_game)
                self.btn_play.clicked.connect(Main_UZ.hide)

                self.retranslateUi(Main_UZ)
                QtCore.QMetaObject.connectSlotsByName(Main_UZ)

        def retranslateUi(self, Main_UZ):
                _translate = QtCore.QCoreApplication.translate
                Main_UZ.setWindowTitle(_translate("Main_UZ", "หมวด U-Z"))
                self.btn_play.setToolTip(_translate("Main_UZ", "<html><head/><body><p>กดปุ่มเพื่อเล่นสุ่มคำศัพท์</p></body></html>"))
                self.btn_save.setToolTip(_translate("Main_UZ", "<html><head/><body><p>กดปุ่มเพื่อบันทึกคำศัพท์</p></body></html>"))
                self.btn_delete.setToolTip(_translate("Main_UZ", "<html><head/><body><p>กดปุ่มเพื่อลบคำศัพท์</p></body></html>"))
                self.btn_play2.setText(_translate("Main_UZ", "PLAY"))
                self.btn_save2.setText(_translate("Main_UZ", "ADD&&SAVE"))
                self.btn_delete2.setText(_translate("Main_UZ", "DELETE"))
                self.label.setText(_translate("Main_UZ", "หมวด U-Z"))
                self.btn_show.setText(_translate("Main_UZ", "SHOW"))
                self.textbox_delete.setPlaceholderText(_translate("Main_UZ", "ตัวอย่าง ant=มด **ใส่=ด้วย"))
                self.btn_search.setToolTip(_translate("Main_UZ", "<html><head/><body><p>กดปุ่มเพื่อค้นหา</p></body></html>"))
                self.textbox_save.setPlaceholderText(_translate("Main_UZ", "ตัวอย่าง ant=มด **ใส่=ด้วย"))
                self.textbox_search.setPlaceholderText(_translate("Main_UZ", "SEARCH"))
                self.btn_back.setToolTip(_translate("Main_UZ", "<html><head/><body><p>กดปุ่มเพื่อกลับหน้าแรก</p></body></html>"))
                                ...
                                if filesize == 0:
                                        ...
                                        with open(file,encoding="UTF-8") as f:
                                                for line in f:
                                                        line = line.replace('\n','')
                                                        (Vocab1,Vocab2) = line.split('=')
                                                        u_z.setdefault(Vocab1,[])
                                                        u_z[Vocab1].append(Vocab2)
