...
class Ui_Main_FJ(object):

        def play_game(self):
                from play_fj import Ui_Play_FJ
        ...
        def setupUi(self, Main_FJ):
                Main_FJ.setObjectName("Main_FJ")
                Main_FJ.resize(1920, 1080)
                icon = QtGui.QIcon()
                icon.addPixmap(QtGui.QPixmap(":/image/Picture/cards.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                Main_FJ.setWindowIcon(icon)
                Main_FJ.setStyleSheet("background-image: url(:/image/Picture/f_j.jpg);")
                self.centralwidget = QtWidgets.QWidget(Main_FJ)
        ...
        "background: transparent")
                ...
                Main_FJ.setCentralWidget(self.centralwidget)
                self.menubar = QtWidgets.QMenuBar(Main_FJ)
                self.menubar.setGeometry(QtCore.QRect(0, 0, 1920, 26))
                self.menubar.setObjectName("menubar")
                Main_FJ.setMenuBar(self.menubar)
                self.statusbar = QtWidgets.QStatusBar(Main_FJ)
                self.statusbar.setObjectName("statusbar")
                Main_FJ.setStatusBar(self.statusbar)
                ...
                self.btn_back.clicked.connect(Main_FJ.hide)
                self.btn_play.clicked.connect(self.play_game)
                self.btn_play.clicked.connect(Main_FJ.hide)

                self.retranslateUi(Main_FJ)
                QtCore.QMetaObject.connectSlotsByName(Main_FJ)

        def retranslateUi(self, Main_FJ):
                _translate = QtCore.QCoreApplication.translate
                Main_FJ.setWindowTitle(_translate("Main_FJ", "หมวด F-J"))
                self.btn_play.setToolTip(_translate("Main_FJ", "<html><head/><body><p>กดปุ่มเพื่อเล่นสุ่มคำศัพท์</p></body></html>"))
                self.btn_save.setToolTip(_translate("Main_FJ", "<html><head/><body><p>กดปุ่มเพื่อบันทึกคำศัพท์</p></body></html>"))
                self.btn_delete.setToolTip(_translate("Main_FJ", "<html><head/><body><p>กดปุ่มเพื่อลบคำศัพท์</p></body></html>"))
                self.btn_play2.setText(_translate("Main_FJ", "PLAY"))
                self.btn_save2.setText(_translate("Main_FJ", "ADD&&SAVE"))
                self.btn_delete2.setText(_translate("Main_FJ", "DELETE"))
                self.label.setText(_translate("Main_FJ", "หมวด F-J"))
                self.btn_show.setText(_translate("Main_FJ", "SHOW"))
                self.textbox_delete.setPlaceholderText(_translate("Main_FJ", "ตัวอย่าง ant=มด **ใส่=ด้วย"))
                self.btn_search.setToolTip(_translate("Main_FJ", "<html><head/><body><p>กดปุ่มเพื่อค้นหา</p></body></html>"))
                self.textbox_save.setPlaceholderText(_translate("Main_FJ", "ตัวอย่าง ant=มด **ใส่=ด้วย"))
                self.textbox_search.setPlaceholderText(_translate("Main_FJ", "SEARCH"))
                self.btn_back.setToolTip(_translate("Main_FJ", "<html><head/><body><p>กดปุ่มเพื่อกลับหน้าแรก</p></body></html>"))
        ...
        def show(self):
                ...
                with open(file,encoding="UTF-8") as f:
                        filesize = os.path.getsize("f_j.txt")
                        if filesize == 0:
                                ...
                                if filesize == 0:
                                        ...
                                        with open(file,encoding="UTF-8") as f:
                                                for line in f:
                                                        line = line.replace('\n','')
                                                        (Vocab1,Vocab2) = line.split('=')
                                                        f_j.setdefault(Vocab1,[])
                                                        f_j[Vocab1].append(Vocab2)
