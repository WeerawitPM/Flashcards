...
class Ui_Play_KO(object):
        ...
        def setupUi(self, Play_KO):
                Play_KO.setObjectName("Play_KO")
                Play_KO.resize(1920, 1080)
                icon = QtGui.QIcon()
                icon.addPixmap(QtGui.QPixmap(":/image/Picture/cards.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                Play_KO.setWindowIcon(icon)
                Play_KO.setStyleSheet("background-image: url(:/image/Picture/play_menu.jpg);")
                self.centralwidget = QtWidgets.QWidget(Play_KO)
        ...
        "background: transparent;")
                ...
                Play_KO.setCentralWidget(self.centralwidget)
                self.menubar = QtWidgets.QMenuBar(Play_KO)
                self.menubar.setGeometry(QtCore.QRect(0, 0, 1920, 26))
                self.menubar.setObjectName("menubar")
                Play_KO.setMenuBar(self.menubar)
                self.statusbar = QtWidgets.QStatusBar(Play_KO)
                self.statusbar.setObjectName("statusbar")
                Play_KO.setStatusBar(self.statusbar)

                self.btn_back.clicked.connect(self.back_menu)
                self.btn_back.clicked.connect(Play_KO.hide)
                ...
                self.retranslateUi(Play_KO)
                QtCore.QMetaObject.connectSlotsByName(Play_KO)

        def retranslateUi(self, Play_KO):
                _translate = QtCore.QCoreApplication.translate
                Play_KO.setWindowTitle(_translate("Play_KO", "Play K-O"))
                self.btn_play.setToolTip(_translate("Play_KO", "<html><head/><body><p>กดปุ่มเพื่อเล่นสุ่มคำศัพท์</p></body></html>"))
                self.btn_correct.setText(_translate("Play_KO", "CORRECT"))
                self.btn_back.setToolTip(_translate("Play_KO", "<html><head/><body><p>กดปุ่มเพื่อกลับหน้าแรก</p></body></html>"))
                self.btn_show.setText(_translate("Play_KO", "SHOW ANSWER"))
                self.btn_incorrect.setText(_translate("Play_KO", "INCORRECT"))
                self.label_question.setText(_translate("Play_KO", "Question"))
                self.label_answer.setText(_translate("Play_KO", "Answer"))
                self.label_score.setText(_translate("Play_KO", "0"))
                        ...
                        if filesize == 0:
                                ...
                                with open(file,encoding="UTF-8") as f:
                                        for line in f:
                                                ...
                                                k_o.setdefault(Question,[])
                                                k_o[Question].append(Answer)
                                        random_ae = random.choice(list(k_o.items()))
                                ans = ", "
